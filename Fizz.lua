...
do
      ...
      local Files = {
            Lua = {
                  Path = SCRIPT_PATH,
                  Name = "Fizz.lua",
                  Url = "https://raw.githubusercontent.com/miragessee/GoSFizz/master/Fizz.lua"
            },
            Version = {
                  Path = SCRIPT_PATH,
                  Name = "miragesfizz.version",
                  Url = "https://raw.githubusercontent.com/miragessee/GoSFizz/master/miragesfizz.version"
            }
      }
...
function WDmgPassive()
	if myHero:GetSpellData(_W).level == 0 then
		local level = 1
        local wdamage = (({20, 30, 40, 50, 60})[level] + 0.40 * myHero.ap)
	    return wdamage
    else
...
function WDmgActive()
	if myHero:GetSpellData(_W).level == 0 then
		local level = 1
        local wdamage = (({50, 70, 90, 110, 130})[level] + 0.50 * myHero.ap)
	    return wdamage
    else
		local level = myHero:GetSpellData(_W).level
        local wdamage = (({50, 70, 90, 110, 130})[level] + 0.50 * myHero.ap)
...
function WDmgActiveHit()
	if myHero:GetSpellData(_W).level == 0 then
		local level = 1
        local wdamage = (({10, 15, 20, 25, 30})[level] + 0.35 * myHero.ap)
	    return wdamage
    else
		local level = myHero:GetSpellData(_W).level
        local wdamage = (({10, 15, 20, 25, 30})[level] + 0.35 * myHero.ap)
...
function EDmg()
	if myHero:GetSpellData(_E).level == 0 then
		local Dmg1 = (({70, 120, 170, 220, 270})[1] + 0.75 * myHero.ap)
    	return Dmg1
    else
		local Dmg1 = (({70, 120, 170, 220, 270})[myHero:GetSpellData(_E).level] + 0.75 * myHero.ap)
    	return Dmg1
	end
end
...
function RDmgGuppy()
	if myHero:GetSpellData(_R).level == 0 then
		local Dmg1 = (({150, 250, 350})[1] + 0.8 * myHero.ap)
    	return Dmg1
    else
		local Dmg1 = (({150, 250, 350})[myHero:GetSpellData(_R).level] + 0.8 * myHero.ap)
    	return Dmg1
	end
end

function RDmgChomper()
	if myHero:GetSpellData(_R).level == 0 then
		local Dmg1 = (({225, 325, 425})[1] + 1.0 * myHero.ap)
    	return Dmg1
    else
		local Dmg1 = (({225, 325, 425})[myHero:GetSpellData(_R).level] + 1.0 * myHero.ap)
    	return Dmg1
	end
end

function RDmgGigalodon()
	if myHero:GetSpellData(_R).level == 0 then
		local Dmg1 = (({300, 400, 500})[1] + 1.20 * myHero.ap)
    	return Dmg1
    else
		local Dmg1 = (({300, 400, 500})[myHero:GetSpellData(_R).level] + 1.20 * myHero.ap)
    	return Dmg1
	end
end
...
local HeroIcon = "https://www.mobafire.com/images/champion/icon/fizz.png"
...
function Fizz:LoadMenu()
	...
	self.FizzMenu:MenuElement({id = "LastHit", name = "LastHit", type = MENU})
	self.FizzMenu.LastHit:MenuElement({id = "UseW", name = "Use W minion kill", value = true, leftIcon = WIcon})

	self.FizzMenu:MenuElement({id = "LaneClear", name = "LaneClear", type = MENU})
	self.FizzMenu.LaneClear:MenuElement({id = "UseW", name = "Use W minion kill", value = true, leftIcon = WIcon})

	self.FizzMenu:MenuElement({id = "Combo", name = "Combo", type = MENU})
	self.FizzMenu.Combo:MenuElement({id = "UseQ", name = "Use Q", value = true, leftIcon = QIcon})
	self.FizzMenu.Combo:MenuElement({id = "UseW", name = "Use W", value = true, leftIcon = WIcon})
	self.FizzMenu.Combo:MenuElement({id = "UseE", name = "Use E", value = true, leftIcon = EIcon})
	self.FizzMenu.Combo:MenuElement({id = "UseR", name = "Use R", value = false, leftIcon = RIcon})
	self.FizzMenu.Combo:MenuElement({id = "UseRC", name = "Use R is only chomper damage", value = false, leftIcon = RIcon})
	...
	self.FizzMenu.KillSteal:MenuElement({id = "UseHG", name = "Use Hextech Gunblade", value = true, leftIcon = HGIcon})
	...
	self.FizzMenu:MenuElement({id = "Escape", name = "Escape", type = MENU})
	self.FizzMenu.Escape:MenuElement({id = "UseE", name = "Use E", value = true})
	self.FizzMenu.Escape:MenuElement({id = "UseHP", name = "Use Hextech Probelt", value = true})

	self.FizzMenu:MenuElement({id = "Drawings", name = "Drawings", type = MENU})
	self.FizzMenu.Drawings:MenuElement({id = "DrawQ", name = "Draw Q Range", value = true})
	self.FizzMenu.Drawings:MenuElement({id = "DrawE", name = "Draw E Range", value = false})
	self.FizzMenu.Drawings:MenuElement({id = "DrawRC", name = "Draw RC Range", value = true})
	self.FizzMenu.Drawings:MenuElement({id = "DrawRG", name = "Draw RG Range", value = true})
	self.FizzMenu.Drawings:MenuElement({id = "DrawAA", name = "Draw Killable AAs", value = false})
	self.FizzMenu.Drawings:MenuElement({id = "DrawKS", name = "Draw Killable Skills", value = true})
	self.FizzMenu.Drawings:MenuElement({id = "DrawJng", name = "Draw Jungler Info", value = true})

	self.FizzMenu:MenuElement({id = "blank", type = SPACE , name = ""})
	self.FizzMenu:MenuElement({id = "blank", type = SPACE , name = "Script Ver: "..Version.. " - LoL Ver: "..LVersion.. ""})
	self.FizzMenu:MenuElement({id = "blank", type = SPACE , name = "by "..Author.. ""})
end
...
function Fizz:LoadSpells()
	...
	FizzE = { range = 400  }
	FizzETwo = { range = 400  }
	FizzR = { radius = 80, delay = 0.25, speed = 1300, range = 1300  }
	FizzRC = { targetRangeMin = 455, targetRangeMax = 910, range = 1300, radius = 80, delay = 0.25, speed = 1300 }
    FizzRG = { targetRangeMin = 910, targetRangeMax = 1300, range = 1300, radius = 80, delay = 0.25, speed = 1300 }
...
function Fizz:Tick()
	...
	if self.FizzMenu.AutoLevel.AutoLevel:Value() then
		...
		if mylevelpts > 0 then
			...
			elseif mylevel == 2 or mylevel == 4 or mylevel == 5 or mylevel == 7 or mylevel == 9 then
				LocalControlKeyDown(HK_LUS)
          		...
          		LocalControlKeyUp(HK_LUS)
			elseif mylevel == 3 or mylevel == 14 or mylevel == 15 or mylevel == 16 or mylevel == 17 or mylevel == 18 then
				LocalControlKeyDown(HK_LUS)
...
function Fizz:CollisionX(myHeroPos, dangerousPos, unitPos, radius)
...
function Fizz:CalculateEndPos(startPos, placementPos, unitPos, range)
	if range > 0 then
...
function Fizz:ProcessSpell(units)
	for i = 1, #units do
		local unit = units[i]
...
function Fizz:KillSteal()
	if self.FizzMenu.KillSteal.UseIgnite:Value() then
	...
	if self.FizzMenu.KillSteal.UseHP:Value() then
		if GetItemSlot(myHero, 3152) > 0 then
			for i,enemy in pairs(GetEnemyHeroes()) do
	...
	for key, v in pairs(IS) do
		...
		if GetDistanceSqr(SpellHit,SpellPosition) <= GetDistanceSqr(dodge,SpellPosition) and GetDistance(SpellHit,v.sPos) - v.radius - myHero.boundingRadius <= GetDistance(v.sPos,v.ePos) then
			if GetDistanceSqr(myHero.pos,SpellHit) < (v.radius + myHero.boundingRadius) ^ 2 then
				if self.FizzMenu.Escape.UseE:Value() then
					if IsReady(_E) then
				...
				if self.FizzMenu.Escape.UseHP:Value() then
					if GetItemSlot(myHero, 3152) > 0 then
						if myHero:GetSpellData(GetItemSlot(myHero, 3152)).currentCd == 0 then
							local castPos = myHero.pos + Vector(myHero.pos,v.sPos):Normalized() * 100
							Control.CastSpell(Item_HK[GetItemSlot(myHero, 3152)], myHero.pos)
						end
					end
				end
			end
...
function Fizz:Draw()
	...
	if self.FizzMenu.Drawings.DrawQ:Value() then Draw.Circle(myHero.pos, FizzQ.range, 1, Draw.Color(255, 0, 191, 255)) end
	if self.FizzMenu.Drawings.DrawE:Value() then Draw.Circle(myHero.pos, FizzE.range, 1, Draw.Color(255, 65, 105, 225)) end
	if self.FizzMenu.Drawings.DrawRC:Value() then Draw.Circle(myHero.pos, FizzRC.range, 1, Draw.Color(255, 30, 144, 255)) end
	if self.FizzMenu.Drawings.DrawRG:Value() then Draw.Circle(myHero.pos, FizzRG.range, 1, Draw.Color(255, 0, 0, 255)) end

	for i, enemy in pairs(GetEnemyHeroes()) do
		if self.FizzMenu.Drawings.DrawJng:Value() then
		...
		if self.FizzMenu.Drawings.DrawAA:Value() then
			if ValidTarget(enemy) then
		...
		if self.FizzMenu.Drawings.DrawKS:Value() then
			if ValidTarget(enemy) then
				...
				elseif enemy.health < (QDmg() + EDmg() + WDmgActive()) then
					Draw.Text("Killable Skills (Q+E+W): ", 25, enemy.pos2D.x-38, enemy.pos2D.y+10, Draw.Color(0xFFFF0000))
				elseif enemy.health < (QDmg() + EDmg() + RDmgGuppy()) then
					Draw.Text("Killable Skills (Q+E+RGuppy): ", 25, enemy.pos2D.x-38, enemy.pos2D.y+10, Draw.Color(0xFFFF0000))
				elseif enemy.health < (QDmg() + EDmg() + RDmgChomper()) then
					Draw.Text("Killable Skills (Q+E+RChomper): ", 25, enemy.pos2D.x-38, enemy.pos2D.y+10, Draw.Color(0xFFFF0000))
				elseif enemy.health < (QDmg() + EDmg() + RDmgGigalodon()) then
					Draw.Text("Killable Skills (Q+E+RGigalodon): ", 25, enemy.pos2D.x-38, enemy.pos2D.y+10, Draw.Color(0xFFFF0000))
				elseif enemy.health < (QDmg() + EDmg() + WDmgActive() + RDmgGigalodon()) then
					Draw.Text("Killable Skills (Q+E+W+RGigalodon): ", 25, enemy.pos2D.x-38, enemy.pos2D.y+10, Draw.Color(0xFFFF0000))
				end
			end
		end
	end
...
function Fizz:LaneClear()
	if self.FizzMenu.LaneClear.UseW:Value() then
		for i = 1, LocalGameMinionCount() do
			local minion = LocalGameMinion(i)
			if minion and minion.isEnemy then
				if IsReady(_W) then
...
function Fizz:Harass()
	if self.FizzMenu.Harass.UseW:Value() then
		for i = 1, LocalGameMinionCount() do
			local minion = LocalGameMinion(i)
			if minion and minion.isEnemy then
				if IsReady(_W) then
...
function Fizz:Combo()
	local targetHP = GOS:GetTarget(850,"AP")

	if self.FizzMenu.Combo.UseHP:Value() then
		if GetItemSlot(myHero, 3152) > 0 and ValidTarget(targetHP, 850) then
			if myHero:GetSpellData(GetItemSlot(myHero, 3152)).currentCd == 0 then
				Control.CastSpell(Item_HK[GetItemSlot(myHero, 3152)], targetHP)
			end
		end
	end
	...
	local targetE = GOS:GetTarget(FizzE.range,"AP")
	local targetR = GOS:GetTarget(FizzR.range,"AP")
	local targetRC = GOS:GetTarget(FizzRC.targetRangeMax,"AP")
...
function Fizz:CastRG(target,RGcastPos)
	if LocalGameTimer() - OnWaypoint(target).time > 0.05 and (LocalGameTimer() - OnWaypoint(target).time < 0.125 or LocalGameTimer() - OnWaypoint(target).time > 1.25) then
		if ValidTarget(target, FizzRG.targetRangeMax) and GetDistance(myHero.pos,RGcastPos) <= FizzRG.targetRangeMax then
			...
			Control.KeyUp(HK_R)
			SetMovement(true)
			--LocalControlCastSpell(HK_R,RGcastPos)
		end
	end
end
